...
### 0.3 同步机制（Git Post-Commit Hook）

**每次 `git commit` 后自动同步到运行目录。** 无需手动操作。

**原理：** `.githooks/post-commit` 检测本次 commit 改动了哪些 skill 目录，rsync 到 `~/.openclaw/skills/`。

- 只同步有 `SKILL.md` 的目录（真正的 skill）
- 保留 `config.json` 和 `state.json`（用户配置和状态不被覆盖）
- 纯 bash，零外部依赖
...
**手动全量同步（可选，一次性同步所有公开 skill）：**
```bash
cd ~/Code/openclaw-skills
for d in */; do
  [ -f "$d/SKILL.md" ] && rsync -a \
...
2. `git add` + `git commit` → **自动同步到 `~/.openclaw/skills/`**
3. `git push`
