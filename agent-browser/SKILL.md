...
## Prerequisites
1. Install agent-browser: check with `agent-browser --version`
2. Install Playwright browsers: `agent-browser install` (first time only)
3. For iOS testing: macOS + Xcode + `npm install -g appium && appium driver install xcuitest`
4. For proxy/geo features: configure proxy settings in agent-browser config
...
agent-browser snapshot -i
# Output: @e1 [input name="email"], @e2 [input name="message"], @e3 [button] "Send"
agent-browser fill @e1 "user@example.com"
