...
1. 激活虚拟环境并检查 session（以下为 Bash/Zsh，Windows PowerShell 见 `knowledge/runtime-requirements.md`）：
   ```bash
...
2. 读取 `~/.openclaw/skills/xhs-publisher/state.json`，检查今日已发数量和上次发帖时间
...
将本次发帖记录追加到 `~/.openclaw/skills/xhs-publisher/state.json`。状态格式见 [`knowledge/safety-policy.md`](knowledge/safety-policy.md)。
